...
## ## Partial by design
##
## A signature that cannot be spelled — a type variable of an open generic —
## is emitted as a `pointer` field rather than guessed at, so the methods after
## it still line up and a caller can cast it by hand. Emitting a *wrong* ABI
## type would be worse than emitting none.
...
proc emitModule(md: WinMd; iids: Table[int, string]; winmdPath, prefix,
  ...
  buf.add "## Each interface is its vtable: an object whose fields are the\n"
  buf.add "## methods in declaration order, after IInspectable's six (IUnknown's\n"
  buf.add "## three for a delegate). Every method returns HRESULT and its\n"
  buf.add "## declared return type becomes a trailing out-parameter.\n\n"
  ...
  for t in md.types:
    ...
    # The vtable, as the C headers spell it: the inherited methods come from
    # the base object, and each declared method is a field in order.
    let parent = if delegate: "IUnknownVtbl" else: "IInspectableVtbl"
    buf.add &"type {ident}Vtbl* = object of {parent}\n"
