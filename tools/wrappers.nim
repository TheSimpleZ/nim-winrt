...
import std/[os, algorithm, strformat, strutils, tables, sets, times]
import ./winmd
import ./foreign
import ./piid
...
func escapeIdent(s: string): string =
  ## A generated name that collides with a Nim keyword is spelled in backticks.
...
when isMainModule:
  ...
  type DelegateInfo = object
    args: seq[SigType]
  var delegates = initTable[string, DelegateInfo]()
  for t in md.types:
    ...
    for mi in first ..< stop:
      ...
      delegates[t.fullName] = DelegateInfo(args: md.methodSignature(mi).params)
      break
        ...
        if raw.startsWith("add_"):
          ...
          if sigE.params.len == 1:
            ...
            if h.kind == skInterface and h.name in delegates:
              handlerArgs = delegates[h.name].args
